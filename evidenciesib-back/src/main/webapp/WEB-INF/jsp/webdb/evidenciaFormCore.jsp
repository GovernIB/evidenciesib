      ...
      <script type="text/javascript">
        ...
        <tr id="evidencia_loginType_rowid">
          ...
          <td id="evidencia_loginType_columnvalueid">
            ...
            <form:input readonly="${ gen:contains(__theForm.readOnlyFields ,EvidenciaFields.LOGINTYPE)? 'true' : 'false'}" cssClass="w-100 form-control  ${gen:contains(__theForm.readOnlyFields ,EvidenciaFields.LOGINTYPE)? ' uneditable-input' : ''}"  style="" maxlength="255" path="evidencia.loginType"   />
        ...
        <tr id="evidencia_loginQaa_rowid">
          ...
          <c:if test="${gen:contains(__theForm.readOnlyFields ,EvidenciaFields.LOGINQAA)}" >
          <form:hidden path="evidencia.loginQaa"/>
          <input type="text" readonly="true" class="form-control col-md-9-optional uneditable-input" value="${gen:findValue(__theForm.evidencia.loginQaa,__theForm.listOfValuesForLoginQaa)}"  />
          </c:if>
          <c:if test="${!gen:contains(__theForm.readOnlyFields ,EvidenciaFields.LOGINQAA)}" >
          <c:set var="containEmptyValue"  value="false" />
          <form:select id="evidencia_loginQaa"  onchange="if(typeof onChangeLoginQaa == 'function') {  onChangeLoginQaa(this); };"  cssClass="form-control col-md-9-optional" path="evidencia.loginQaa">
            <c:forEach items="${__theForm.listOfValuesForLoginQaa}" var="tmp">
                <form:option value="${tmp.key}">${tmp.value}</form:option>
                <c:if test="${empty tmp.key}">
                  <c:set var="containEmptyValue"  value="true" />
                </c:if>
            ...
            <c:if test="${not containEmptyValue}">
              <c:if test="${empty __theForm.evidencia.loginQaa }">
                  <form:option value="" selected="true" ></form:option>
              </c:if>
              <c:if test="${not empty __theForm.evidencia.loginQaa }">
