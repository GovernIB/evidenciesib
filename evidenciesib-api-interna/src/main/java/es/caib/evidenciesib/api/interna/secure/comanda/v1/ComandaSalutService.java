...
import es.caib.evidenciesib.ejb.PluginService;
...
public class ComandaSalutService extends RestUtils implements es.caib.comanda.api.server.monitoring.ComandaAppSalutApi {
    ...
    public static final String SUBSISTEMA_EVIDENCIA = "EVI_EVIDENCIA";

    {
        subsistemaNameById.put(SUBSISTEMA_EVIDENCIA, "Evidencies");


        subsistemaTipusOkById.put(SUBSISTEMA_EVIDENCIA,
               EvidenciaFields.ESTATCODI.equal(Constants.EVIDENCIA_ESTAT_CODI_SIGNAT));

        subsistemaTipusErrorById.put(SUBSISTEMA_EVIDENCIA,
                EvidenciaFields.ESTATCODI.equal(Constants.EVIDENCIA_ESTAT_CODI_ERROR));
    ...
    public AppInfo salutInfo() {
        ...
        String urlBase = Configuracio.getBackUrl().replaceAll("/evidenciesibback","");

        a.codi("EVI");
        a.nom("EvidènciesIB (Firma Àgil)");

        {
            ...
            List<ContextInfo> contexts = new ArrayList<>();
            {
                ...
                back.setApi(null);
                back.setCodi("EVI_BACK");
                // TODO Falta Manual
                /*
                Manual manual = new Manual();
                manual.setNom("Manual_de_Usuari_de_Utilitats de Firma");
                manual.setPath(
                        "https://github.com/GovernIB/evidenciesib/raw/refs/heads/evidenciesib-3.0/doc/Manual_de_Usuari_de_Utilitats de Firma.odt");
                back.setManuals(List.of(manual));
                */
                back.setNom("Utilitats de Firma Backoffice/Frontoffice");
                back.setPath(urlBase + "/evidenciesibback");
...

            {
                ...
                manual.setPath(
                        "https://github.com/GovernIB/evidenciesib/raw/refs/heads/evidenciesib-3.0/doc/Manual_de_Migraci%C3%B3_de_APIsIB_a_Api_Interna.odt");
...
*/                
...

        {
            ...
            for (Map.Entry<String, String> entry : subsistemaNameById.entrySet()) {
                ...
                subSystemApiFirmaAsinc.setTotalError(
                        calculPeticions( faunany, avui, subsistemaTipusErrorById.get(codiSubsistema)));
                subSystemApiFirmaAsinc.setTotalOk(
                        calculPeticions(faunany, avui, subsistemaTipusOkById.get(codiSubsistema)));
    ...
    protected long calculPeticions(Timestamp from, Timestamp to, Where whereTipus) {
        ...
        Where w1 = EvidenciaFields.USUARIAPLICACIO.isNotNull();
        Where w2 = EvidenciaFields.DATAINICI.between(from, to);

        Where w = Where.AND(w1, w2, whereTipus);
        try {
            totalOK = evidenciaLogicaEjb.count(w);
        } catch (I18NException e) {
